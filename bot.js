  ...
  bot.on('message', async msg => {
    ...
    if (text === '/start') {
      if (!userStates[chatID] || !userStates[chatID].groupNumber) {
        ...
        await bot.sendMessage(chatID, `Здравствуйте ${msg.from.first_name}, вас не приветствует КГТУбот, здесь вы можете смотреть расписание для вашей группы. Введите номер своей группы и используйте меню, чтобы изменить номер группы`);
      } else {
